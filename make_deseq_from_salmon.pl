...
# PODNAME: make_deseq_from_salmon.pl
# ABSTRACT: Make DESeq2 counts and samples files from Salmon files
...
foreach my $count_file (@count_files) {
    ...
    my ($sample) = $filename =~ m/\A (.*) [.]count/xms;
...
GENE: while (1) {
    ...
    foreach my $sample (@samples) {
        ...
        chomp $line;
        my ( $gene, undef, undef, undef, $count ) = split /\t/xms, $line;
        $count = int( $count + 0.5 );     ## no critic (ProhibitMagicNumbers)
...
make_deseq_from_salmon.pl

Make DESeq2 counts and samples files from Salmon files
...
This script takes a list of Salmon output files and creates merged counts.txt
and samples.txt files for DESeq2 in the current directory. Additionally,
Sequencescape sample names (e.g. 3407STDY6401339) are replaced with more
readable sample names (e.g. zmp_ph250_E4_p2).
...
=head1 EXAMPLES
    ...
    perl \
        make_deseq_from_salmon.pl --count_files somites.fofn
...
Salmon output files. If just one file is specified then it's assumed to be a
file of filenames.
