...
with(data, plot(log2fc, -log10(adjp), pch=20, main="Volcano plot",
                xlim=c(-2.5,2)))
with(subset(data, adjp < 0.05 ), points(adjp, -log10(adjp), pch=20, col="red"))
with(subset(data, abs(log2fc) > 1), points(adjp, -log10(adjp), pch=20,
                                         col="orange"))
with(subset(data, adjp < 0.05 & abs(log2fc) > 1), points(adjp, -log10(adjp),
                                                         pch=20, col="green"))
graphics.off()
